...
export function AdminMarketplacePage() {
  ...
  const handleExportData = (selectedIds: string[], dateRange: { from: string; to: string }) => {
    const allListings = [...listings.pending, ...listings.approved];
    const dataToExport = allListings.filter(listing => selectedIds.includes(listing.id));
    
    // Apply date filter if provided (using submittedAt as date field)
    let filteredItems = dataToExport;
    if (dateRange?.from || dateRange?.to) {
      filteredItems = dataToExport.filter(listing => {
        // Since these are mock data, we'll use current date approximation
    ...
    console.log('Exported marketplace listings to CSV');
  };
  ...
  const bulkApprove = () => {
    ...
    if (confirmed) {
      ...
      console.log(`Bulk approved ${pendingIds.length} listings at ${new Date().toISOString()}`);
    }
  };
  ...
  const approveListing = (listingId: string) => {
    ...
    if (listing) {
      ...
      if (confirmation) {
        ...
        console.log(`Listing ${listingId} approved by admin at ${new Date().toISOString()}`);
      }
    }
  };
  ...
  const reviewListing = (listingId: string) => {
    ...
    if (listing) {
      const listingDetails = `LISTING REVIEW:\n\nTitle: ${listing.title}\nSeller: ${listing.seller}\nPrice: AED ${listing.price.toLocaleString()}\nCategory: ${listing.category}\nLocation: ${listing.location}\nImages: ${listing.images}\nStatus: ${listing.status}\n\nDescription: ${listing.description || 'N/A'}\n\nBoost Package: ${listing.boostPackage || 'None'}\nViews: ${listing.views || 0}\nInquiries: ${(listing as any).inquiries || 0}\n\nWould you like to take action on this listing?`;
      
      const takeAction = window.confirm(listingDetails);
      if (takeAction) {
        toast.info(`📋 Reviewing listing: ${listing.title}`);
      }
    }
  };
  ...
  const rejectListing = (listingId: string) => {
    ...
    if (listing) {
      ...
      if (reason) {
        ...
        console.log(`Listing ${listingId} rejected by admin. Reason: ${reason}. Time: ${new Date().toISOString()}`);
      }
    }
  };
  ...
  const editListing = (listingId: string) => {
    ...
    if (listing) {
      ...
      if (newTitle && newPrice) {
        ...
        console.log(`Listing ${listingId} edited by admin at ${new Date().toISOString()}`);
      }
    }
  };
  ...
  const suspendListing = (listingId: string) => {
    ...
    if (listing) {
      ...
      if (reason) {
        ...
        console.log(`Listing ${listingId} suspended by admin. Reason: ${reason}. Time: ${new Date().toISOString()}`);
      }
    }
  };
  ...
  const editBoostPackage = (packageType: string) => {
    ...
    if (newPrice && newDuration) {
      ...
      console.log(`${packageType} boost package updated:`, { price: newPrice, duration: newDuration, timestamp: new Date().toISOString() });
    }
  };
  ...
  const exportListings = () => {
    ...
    const headers = ['ID', 'Title', 'Seller', 'Category', 'Price', 'Location', 'Status', 'Views', 'Images', 'Submitted At'];
    const csvContent = [
      headers.join(','),
    ...
    toast.success(`✅ Exported ${allListings.length} listings to CSV`);
    console.log(`Exported ${allListings.length} marketplace listings to CSV at ${new Date().toISOString()}`);
  };
  ...
  const renderListingsContent = () => (
    <div className="space-y-6">
      ...
      <DateRangeFilter 
        ...
        totalItems={[...listings.pending, ...listings.approved].length}
      />
          ...
          {listings.pending.map((listing) => (
            <div key={listing.id} className="bg-[var(--sublimes-card-bg)] border border-orange-500/30 rounded-xl p-6 relative">
              ...
              <div className="flex items-start justify-between mb-4 ml-8">
                ...
                <button className="p-2 text-gray-400 hover:text-[var(--sublimes-light-text)]">
      ...
      {/* Active Listings */}
      <div>
        <h3 className="text-xl font-bold text-[var(--sublimes-light-text)] mb-4">Active Listings</h3>
        <div className="space-y-4">
